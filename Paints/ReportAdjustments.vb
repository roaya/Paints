Public Class ReportAdjustments
    ...
    Private Sub BtnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            Else
                ...
                cmd.CommandText = "select *,(select logo from stocks where stock_id=" & ProjectSettings.CurrentStockID & ") as logo  from Report_Adjustment where Adjustment_id = " & BillID.Text
    ...
    Private Sub ToStockID_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToStockID.SelectedIndexChanged
        Try
            If b = True Then
                cmd.CommandText = "select distinct adjustment_id from Report_Adjustment where to_stock = N'" & ToStockID.Text & "'"
                BillID.Items.Clear()
                dr = cmd.ExecuteReader
                Do While Not dr.Read = False
    ...
    Private Sub FromStockID_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FromStockID.SelectedIndexChanged
        Try
            If b = True Then
                ...
                cmd.CommandText = "select distinct  adjustment_id from Report_Adjustment where from_stock = N'" & FromStockID.Text & "'"
                BillID.Items.Clear()
                dr = cmd.ExecuteReader
                Do While Not dr.Read = False
                    BillID.Items.Add(dr("adjustment_id"))
    ...
    Private Sub ItemName_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles ItemName.Leave
        Try
            If ItemName.SelectedValue = Nothing Then
                cls.MsgExclamation("أدخل اسم الصنف")
            Else
                cmd.CommandText = "select distinct adjustment_id from Report_Adjustment where item_id = " & ItemName.SelectedValue
                BillID.Items.Clear()
                dr = cmd.ExecuteReader
                Do While Not dr.Read = False
                    BillID.Items.Add(dr("adjustment_id"))
    ...
    Private Sub Barcode_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Barcode.Leave
        Try
            cmd.CommandText = "select distinct adjustment_id from Report_Adjustment where barcode = N'" & Barcode.Text & "'"
            BillID.Items.Clear()
            dr = cmd.ExecuteReader
            Do While Not dr.Read = False
                BillID.Items.Add(dr("adjustment_id"))
