...
#Region "Order_Subs"
    ...
    Sub AddItem()
        ...
        ElseIf OrderType.Text = "" Then
            cls.MsgExclamation("أدخل نوع الفاتورة", "please enter order type")
        ...
        If B_EndLoad = True Then
            Try
                ...
                If FirstTaxType.Text = "نسبة مئوية" Then
                    Tx1 = TotalTemp * (FirstTax.Value / 100)
    ...
    Sub Commit_Form()
        Try
            If PayType.Text = "نقدي" Then
                PayedValue.Value = 0
                CashValue.Value = 0
                PayedValue.Value = RemainingValue.Text * -1
                CashValue.Value = PayedValue.Value
            End If
            ...
            Else
                ...
                For i As Integer = 0 To DataGridView1.Rows.Count - 1
                    ...
                    If DataGridView1.Rows(i).Cells("Item_Type").Value = "صنف مخزني" Then
                        cmd.CommandText = "select sum(balance/equivalent_Quantity) from report_items_stocks_balance where um_detail_id=" & DataGridView1.Rows(i).Cells("um_detail_id").Value & " and item_id=" & DataGridView1.Rows(i).Cells("item_id").Value & " and stock_id=" & StockID.SelectedValue
                        If cmd.ExecuteScalar < DataGridView1.Rows(i).Cells("quantity").Value Then
          ...
          TotalBill.Text & ",N'" & DiscountType.Text & "'," & DiscountValue.Value & "," & EmpIDVar & "," & CashValue.Value & "," & CreditValue.Value & ",N'" & PayType.Text & "',N'" & _
                  ...
                  " , " & GeneralNumber.Value & " , " & TotalCost.Text & " , " & TotalProfit.Text & ", N'نسبة مئوية' , N'لا يوجد' ," & ChequeID.Tag & "," & VisaID.Tag & "," & BStatus & "," & Tax.Value & "," & CurrentShiftID
                cmd.ExecuteNonQuery()


                For i As Integer = 0 To DataGridView1.Rows.Count - 1
                    Total_Quantity = DataGridView1.Rows(i).Cells("Quantity").Value
                    While Total_Quantity > 0
                        If BType = "Last" Then
                            cmd.CommandText = "select max(expired_Date) from items_stocks where item_id=" & DataGridView1.Rows(i).Cells("Item_ID").Value & " and stock_id=" & StockID.SelectedValue
                            Exp = cmd.ExecuteScalar
                        Else
                            cmd.CommandText = "select min(expired_Date) from items_stocks where item_id=" & DataGridView1.Rows(i).Cells("Item_ID").Value & " and stock_id=" & StockID.SelectedValue
                            Exp = cmd.ExecuteScalar
                        End If

                        cmd.CommandText = "select balance/equivalent_quantity from report_items_stocks_balance where um_detail_id=" & DataGridView1.Rows(i).Cells("um_detail_id").Value & " and expired_date=(select min(expired_Date) from items_stocks where item_id=" & DataGridView1.Rows(i).Cells("Item_ID").Value & " and stock_id=" & StockID.SelectedValue & ")"
                        Quan = cmd.ExecuteScalar

                        If Quan >= Total_Quantity Then
                            Quan = Total_Quantity
                            Total_Quantity = 0
                        Else
                        ...
                        SER = ProjectSettings.CurrSerialPK
            ...
            "," & Quan & "," & DataGridView1.Rows(i).Cells("Price").Value & "," & _
            "N'" & DataGridView1.Rows(i).Cells("Discount_Type").Value & "'," & DataGridView1.Rows(i).Cells("Discount_Value").Value & "," & DataGridView1.Rows(i).Cells("Price").Value * Quan & _
            "," & StockID.SelectedValue & ", '" & Exp.ToString("MM/dd/yyyy") & "' , '" & BDate.ToString("MM/dd/yyyy") & "', N'" & DataGridView1.Rows(i).Cells("Trade_Name").Value & "' ," & _
            " N'" & DataGridView1.Rows(i).Cells("Item_Type").Value & "' , " & SER
                ...
                cls.RefreshData("select * from sales_details where bill_id=" & DataGridView1.Rows(0).Cells("Bill_ID").Value, MyDs.Tables("items_sales_details"))
...
#End Region
    ...
    Private Sub BtnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNew.Click, newB.Click
        Try
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click, saveB.Click
        Commit_Form()

    End Sub

    Private Sub BtnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, delete.Click
    ...
    Private Sub SalesOrderNormal_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If MyDs.Tables("App_preferences").Rows(0).Item("sal_def_Style") = "مبسطة أخر في أول" Then
                BType = "Last"
            Else
                BType = "First"
            End If
    ...
    Private Sub BtnSavePrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSavePrint.Click, saveAndPrint.Click
        CommitPrint()
    End Sub
          ...
          TotalBill.Text & ",N'" & DiscountType.Text & "'," & DiscountValue.Value & "," & EmpIDVar & "," & CashValue.Value & "," & CreditValue.Value & ",N'" & PayType.Text & "',N'" & _
            ...
            " N'" & DataGridView1.Rows(i).Cells("Item_Type").Value & "' , " & DataGridView1.Rows(i).Cells("Serial_PK").Value
                ...
                If MyDs.Tables("App_Preferences").Rows(0).Item("Gen_View_Before_Print") = False Then
                    rptsal.PrintToPrinter(1, True, 0, 0)
                Else
    ...
    Private Sub FirstTax_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles FirstTax.Leave
        'If TotalBill.Text > 0 Then
        CalculateTotalBill()
        'End If
    End Sub

    Private Sub SecondTax_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles TaxManufacture.Leave
        '  If TotalBill.Text > 0 Then
        CalculateTotalBill()
        'End If
    End Sub
